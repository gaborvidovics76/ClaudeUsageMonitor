...
STRINGS: Dict[str, Dict[str, str]] = {
    ...
    "notify.stale_body": {
        ...
        "zh": "最后一次读取是 {} 之前。Claude Desktop 正在运行吗？",
    },
    "notify.login_ok": {
    ...
    "dlg.intro": {
        ...
        "zh": "在你自己的浏览器中登录 claude.ai 账户 (已保存的密码和通行密钥在那里可以直接使用)。",
    },
    "dlg.step1": {
    ...
    "dlg.err_ratelimit": {
        ...
        "zh": "短时间内登录尝试次数过多。\n\n服务器正在临时限制你。请关闭此窗口，等待 10–15 分钟 (期间不要再尝试)，然后用一个全新的代码重新发起一次浏览器登录。",
    },
    "dlg.err_badcode": {
        ...
        "zh": "代码未被接受。\n\n{}\n\n请检查是否粘贴了完整的代码，或者重新在浏览器中登录一次 (每次都要用新的代码)。",
    },
    "dlg.unknown_err": {
    ...
    "set.data_hint": {
        ...
        "zh": "本地日志: 读取 Claude Desktop 的 plan-usage-history.json。无需登录，但只统计本机，大约每 5 分钟刷新一次。\n\nclaude.ai: 登录后会查询服务器。你可以看到所有设备的用量，并获得准确的重置时间和更高的刷新频率。",
    },
    "set.pick_file_title": {
